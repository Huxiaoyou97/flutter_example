/// xb_nine_picture.dart
/// author: Huxiaoyou
/// description:  九宫格图片展示 4图处理 加载本地和网络图片

import 'package:flutter/material.dart';
import 'package:flutter_demo01/common/widgets/xb_photo_browser.dart';
...
class XbNinePicture extends StatelessWidget {
  const XbNinePicture({
    Key? key,
    this.imgData,
    this.lRSpace = 0.0,
  ...
  _itemCell(context, index) {
    ...
    Widget picture =
        img.startsWith('http') ? XbNetworkImage(img, fit: BoxFit.cover) : Image.asset(img, fit: BoxFit.cover);
  ...
  _clickItemCell(context, index) {
    XbPhotoBrowser.show(context, data: imgData!, index: index, onLongPress: onLongPress);
  }
}
