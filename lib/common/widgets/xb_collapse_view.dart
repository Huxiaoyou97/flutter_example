...
class XbCollapseView extends StatefulWidget {
  const XbCollapseView({
    Key? key,
    this.isFold = false,
    this.content,
    this.titleWidget,
    ...
    this.collapseType = XbCollapseType.rightArrow,
    this.collapseStyle = XbCollapseStyle.flat,
    this.onChange,
  }) : super(key: key);
  ...
  final EdgeInsetsGeometry?
      margin; // margin，默认 flat样式: EdgeInsets.symmetric(horizontal: 3, vertical: 8), card样式: EdgeInsets.all(10)，传值则覆盖默认值
  final EdgeInsetsGeometry?
      padding; // padding,默认 flat样式: 0, card样式: 10，传值则覆盖默认值
  final EdgeInsetsGeometry?
      headerPadding; // header padding,默认 flat样式: 10, card样式:展开8,折叠0，传值则覆盖默认值
  ...
  final bool
      hiddenDivider; // 是否隐藏分割线，仅对 XbCollapseType.centerArrow、XbCollapseType.seeMore 生效
  final XbCollapseType collapseType; // 折叠类型
  final XbCollapseStyle collapseStyle; // 折叠样式。flat: 扁平 card: 卡片
  final Function(bool value)? onChange; // 折叠状态改变回调

  @override
  State<XbCollapseView> createState() => _XbCollapseViewState();
}

class _XbCollapseViewState extends State<XbCollapseView> {
  bool _isFold = false;
  ...
  @override
  void didUpdateWidget(covariant XbCollapseView oldWidget) {
  ...
  _body() {
    var bgColor = widget.bgColor ??
        KColors.dynamicColor(
            context, KColors.kCellBgColor, KColors.kCardBgDarkColor);
    var shadowColor =
        KColors.dynamicColor(context, Colors.black12, Colors.white24);
    var cardBorderStyle = BoxDecoration(
      color: bgColor,
      borderRadius: BorderRadius.circular(3),
      boxShadow: [
        BoxShadow(color: shadowColor, spreadRadius: 1.5, blurRadius: 1.5)
      ],
    );

    return Container(
      margin: widget.margin ??
          (widget.collapseStyle == XbCollapseStyle.card
              ? _cardMargin
              : _flatMargin),
      ...
      child: Column(
        ...
        children: [
          widget.collapseType != XbCollapseType.seeMore
              ? _headerW()
              : Container(),
          Visibility(
            visible: !_isFold,
  ...
  _headerW() {
    var headerColor = widget.headerColor ??
        KColors.dynamicColor(
            context, KColors.kCardBgColor, KColors.kCardBgDarkColor);
    ...
    if (widget.collapseType == XbCollapseType.rightArrow) {
      w = Row(
        ...
        children: [
          ...
          Icon(isExpand ? Icons.arrow_drop_down : Icons.arrow_right,
              size: 22, color: widget.arrowColor),
        ],
      );
      padding = EdgeInsets.only(bottom: isExpand ? 8 : 0);
    }
    if (widget.collapseType == XbCollapseType.centerArrow) {
    ...
    if (widget.collapseType == XbCollapseType.seeMore) {
      w = separatorView(isExpand,
          title: widget.title,
          titleStyle: titleStyle,
          arrowColor: widget.arrowColor,
...
Widget separator(
    ...
    EdgeInsetsGeometry? margin = const EdgeInsets.only(top: 2, bottom: 10)}) {
