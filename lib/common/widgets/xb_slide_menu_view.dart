...
class XbSlideMenuView extends StatefulWidget {
  const XbSlideMenuView({
    Key? key,
    required this.data,
    this.maxRow = 2,
    this.maxColumn = 5,
  ...
  @override
  State<XbSlideMenuView> createState() => _XbSlideMenuViewState();
}

class _XbSlideMenuViewState extends State<XbSlideMenuView> {
  var _dataArr = [];
  ...
  _body() {
    ...
    var mainAxisExtent =
        (XbScreenUtils.screenWidth - margin * 2) / widget.maxColumn;
    var bgColor = widget.bgColor ??
        KColors.dynamicColor(
            context, KColors.kCellBgColor, KColors.kCellBgDarkColor);
    return Container(
      margin: EdgeInsets.all(margin),
      decoration: BoxDecoration(
          color: bgColor, borderRadius: BorderRadius.circular(widget.radius)),
      child: Column(
        children: [
          SizedBox(
            height: mainAxisExtent * widget.maxRow * 1.0,
            width: XbScreenUtils.screenWidth,
  ...
  Widget _buildCell(mainAxisExtent, index) {
    ...
    Widget imageWidget = img.startsWith('http')
        ? XbNetworkImage(img, fit: BoxFit.cover)
