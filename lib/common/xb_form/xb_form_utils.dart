...
// 限制只能输入汉字或者字母或数字
final test = FilteringTextInputFormatter(
...
/// 定义策略类
class XbValidate extends _Strategies {
  var rules;
  var params;

  List<Function> validateList = [];

  XbValidate(this.rules, this.params) {
    addRule(rules);
  }
        ...
        if (parsed < min) {
          ...
          return TextEditingValue(
              ...
              selection: TextSelection.collapsed(
                  offset: max.toString().length)); // 输入大于最大值时返回最大值
        } else {
          return newValue;
        }
      }
